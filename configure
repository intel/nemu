...
supported_kvm_target() {
    ...
    case "${1%-softmmu}:$cpu" in
        ...
        x86_64_virt:i386 | x86_64_virt:x86_64 | x86_64_virt:x32 | \
...
for opt do
  ...
  ;;
  --disable-bluetooth) bluetooth="no"
  ;;
  --enable-bluetooth) bluetooth="yes"
  ;;
...
disabled with --disable-FEATURE, default is enabled if available:
  ...
  fdt             fdt device tree
  bluetooth       Bluetooth emulation support
...
echo "bluetooth support $bluetooth"
if test "$bluetooth" = "yes" ; then
    echo "bluez  support    $bluetooth-bluez"
fi
...
if test "$bluetooth" = "yes" ; then
      echo "CONFIG_BT=y" >> $config_host_mak
      if test "$bluez" = "yes" ; then
	  echo "CONFIG_BT_BLUEZ=y" >> $config_host_mak
...
if test "$vvfat" = "yes" ; then
    echo "CONFIG_BLOCK_VVFAT=y" >> $config_host_mak
fi
...
if test "$tcg" = "yes"; then
    QEMU_INCLUDES="-iquote . -iquote \$(SRC_PATH) -iquote \$(SRC_PATH)/accel/tcg -iquote \$(SRC_PATH)/include $QEMU_INCLUDES"

    # Arch specific TCG headers
    ...
    # TCG interpreter header
    if test "$tcg_interpreter" = "yes"; then
        QEMU_INCLUDES="-iquote \$(SRC_PATH)/tcg/tci $QEMU_INCLUDES"
    fi
 else
    QEMU_INCLUDES="-iquote . -iquote \$(SRC_PATH) -iquote \$(SRC_PATH)/include $QEMU_INCLUDES"
fi # tcg = yes
...
if test "$target_name" = x86_64_virt; then
    echo "TARGET_X86_64=y" >> $config_target_mak
fi
...
if test "$wdt" = "yes"; then
echo "CONFIG_WDT=y" >> $config_host_mak
fi
