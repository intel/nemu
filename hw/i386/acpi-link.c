...
static void acpi_zones_init(AcpiConfiguration *conf)
{
    uint64_t start;

    assert(conf->below_4g_mem_size >= ACPI_HIMEM_SIZE);
    start = conf->below_4g_mem_size - ACPI_HIMEM_SIZE;
...
static AcpiFiles *acpi_files;

static void acpi_files_init(void)
{
    acpi_files = g_new(AcpiFiles, 1);
    acpi_files->file_list = g_array_new(false, true /* clear */,
                                        sizeof(AcpiFileEntry));
}

static AcpiFileEntry *acpi_file_search(const char *name)
{
    int i;
    ...
    for (i = 0; i < file_list->len; i++) {
        file = &g_array_index(file_list, AcpiFileEntry, i);
...
{
    AcpiFileEntry file = { g_strdup(name), zone, offset };
    assert(!acpi_file_search(name));
    g_array_append_val(acpi_files->file_list, file);
}

static hwaddr acpi_file_get_addr(AcpiFileEntry *file)
{
...
static void acpi_patch_add_pointer(const BiosLinkerLoaderEntry *cmd,
                                           Error **errp)
{
    AcpiFileEntry *dest_file, *src_file;
    hwaddr dest;
    uint64_t pointer = 0;
...
static void acpi_patch_add_checksum(const BiosLinkerLoaderEntry *cmd,
                                            Error **errp)
{
    AcpiFileEntry *file = acpi_file_search(cmd->cksum.file);
