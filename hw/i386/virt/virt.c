...
static void virt_machine_state_init(MachineState *machine)
{
    ...
    if (vms->fw) {
        ...
        if (vms->hotplug_memory.base) {
            ...
            uint64_t res_mem_end = vms->hotplug_memory.base;

            res_mem_end += memory_region_size(&vms->hotplug_memory.mr);
...
static void
virt_machine_get_hotplug_memory_region_size(Object *obj, Visitor *v,
                                          const char *name, void *opaque,
                                          Error **errp)
{
    VirtMachineState *vms = VIRT_MACHINE(obj);
    int64_t value = memory_region_size(&vms->hotplug_memory.mr);
...
static void virt_class_init(ObjectClass *oc, void *data)
{
    ...
    object_class_property_add(oc, PC_MACHINE_MEMHP_REGION_SIZE, "int",
                              virt_machine_get_hotplug_memory_region_size, NULL,
